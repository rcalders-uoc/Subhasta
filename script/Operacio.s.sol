...
contract CreacioOferta is Script 
{
   ...
   uint256 idSubhasta = vm.envUint("ID_SUBHASTA");
        ...
        bytes memory dadesOferta = abi.encodeWithSignature(
            "novaOferta(uint256)",
            idSubhasta
        );
     ...
     function run() external {
       address contracteNFT =  vm.envAddress("ADRECA_CONTRACTENFT");
        ...
        // Es crida finalitzacio, ho pot cridar qualsevol
             bytes memory dadesFinalitzacio = abi.encodeWithSignature(
            "finalitzacio(uint256)",
            idSubhasta
        );
       (bool success2, ) = 
       subhastaProxy.call(dadesFinalitzacio);   
        vm.stopBroadcast();
